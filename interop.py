...
class ToGymEnv:
    ...
    def reset(self):
        ...
        return multimap(lambda x: x[0], ob), self.env.get_info()[0]
    ...
    def render(self, mode="rgb_array"):
        ...
        if mode == 'human':
            raise UserWarning('Procgen doesn\'t support \'human\' \
                            render mode currently as its backend is gym3! \
                            So, calling render(mode=\'human\') has no effect.')
        info = self.env.get_info()[0]
